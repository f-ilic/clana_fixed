...
def create_weight_matrix(grouping: List[int]) -> npt.ArrayLike:
    """
...
def get_score(cm: npt.ArrayLike, grouping: List[int], lambda_: float) -> float:
    """
...
def find_thres(cm: npt.ArrayLike, percentage: float) -> float:
    """
...
def find_thres_interactive(cm: npt.ArrayLike, labels: List[str]) -> float:
    """
    Find a threshold for grouping.
...
def get_neighboring_connectivity(cm: npt.ArrayLike) -> List[float]:
    """
...
def split_at_con_thres(
    cm: npt.ArrayLike, thres: float, labels: List[str], interactive: bool
) -> List[int]:
    """
